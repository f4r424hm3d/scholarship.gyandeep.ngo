...
@section('main-section')
                              ...
                              <div class="demo-radio-button">
                                <div class="d-block">
                                  <input name="answer" type="radio" id="a" class="with-gap"
                                    value="{{ $ques_det->a }}"
                                    {{ $ques_det->a == $AnswerSheet->answer ? 'checked' : '' }}><label
                                    for="a">{{ $ques_det->a }}</label>
                                </div>
                                <div class="d-block mt-5">
                                  <input name="answer" type="radio" id="b" class="with-gap"
                                    value="{{ $ques_det->b }}"
                                    {{ $ques_det->b == $AnswerSheet->answer ? 'checked' : '' }}><label
                                    for="b">{{ $ques_det->b }}</label>
                                </div>
                                <div class="d-block mt-5">
                                  <input name="answer" type="radio" id="c" class="with-gap"
                                    value="{{ $ques_det->c }}"
                                    {{ $ques_det->c == $AnswerSheet->answer ? 'checked' : '' }}><label
                                    for="c">{{ $ques_det->c }}</label>
                                </div>
                                <div class="d-block mt-5">
                                  <input name="answer" type="radio" id="d" class="with-gap"
                                    value="{{ $ques_det->d }}"
                                    {{ $ques_det->d == $AnswerSheet->answer ? 'checked' : '' }}><label
                                    for="d">{{ $ques_det->d }}</label>
                                </div>
                              </div>
                            </div>
                          </div>
                ...
                <div class="box-footer">
                  ...
                  <div class="pull-right">
                    @if ($next == '')
                      ...
                      {{-- <a href="#yes-no" class="popup-with-form waves-effect waves-light btn btn-success img-fluid">
                        Submit Test
                      </a> --}}
                    @else
                      <a href="javascript:void()" onclick="saveAndNext('save','{{ $next }}')">
          ...
          <div class="rside">
            ...
            @if ($section_id != '')
              <div class="all-borsection">
                <div class="sec">Section: {{ $currentSubject->subject }}</div>
                <div class="num">
  ...
  <form id="yes-no" class="mfp-hide white-popup-block text-center pb-35 w-450">
    ...
    <a onclick="saveAndNext('save','')" href="javascript:void()" class="btn btn-success mr-2">Yes</a>
  ...
  <form id="summary" class="mfp-hide white-popup-block pb-35 w-500">
    ...
    <div class="row test-summary-text pl-35 pr-35">
      ...
      <a class="mfp-close waves-effect waves-light btn btn-danger d-inline-block w-auto h-auto" style="opacity:0">x</a>
    </div>
  </form>
  <script>
    $(document).ready(function() {
